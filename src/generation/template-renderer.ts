...
function renderTypescriptJestSecurity(ctx: GenerationContext): string {
  ...
  const lines: string[] = [
    ...
    `    // RULE-SEC04: OWASP canonical SQL injection strings only`,
    `    const SQL_PAYLOADS = ${JSON.stringify(OWASP_INJECTION_STRINGS.sql)};`,
    ``,
    `    for (const payload of SQL_PAYLOADS) {`,
    `      const response = await request(app)`,
    `        .${method}('${endpoint.pathNormalized}')`,
    `        .set('Authorization', VALID_AUTH_TOKEN)`,
    `        .send({ title: payload, body: 'Body', description: 'Desc' });`,
    ``,
