...
import { generateSecuritySummary, generateFallbackAnalysis } from '../utils/markdownSummaries';
...
export default function SecurityPage() {
  ...
  if (!section) {
    ...
    if (frameworks?.includes('spring') || frameworks?.includes('spring-boot')) {
      stackSuggestions.push('Spring Security: Add tests for @PreAuthorize, filter chains, CSRF protection, JWT validation');
    }
    if (frameworks?.includes('express') || languages?.includes('javascript') || languages?.includes('typescript')) {
      stackSuggestions.push('Express/Node: Add tests for auth middleware, JWT verification, session handling, route guards');
    }
    if (frameworks?.includes('angular')) {
      stackSuggestions.push('Angular: Add tests for route guards, HTTP interceptors, and auth service token handling');
    }
    if (frameworks?.includes('django') || languages?.includes('python')) {
      stackSuggestions.push('Django/Python: Add tests for permission classes, authentication backends, CSRF middleware, and decorators');
    }
    if (frameworks?.includes('rails') || languages?.includes('ruby')) {
      stackSuggestions.push('Rails/Ruby: Add tests for before_action auth filters, Pundit/CanCanCan policies, and session management');
    }
    ...
    return (
      <div className="p-6">
        ...
        {showAiSummaries && report && (
          <AiSummaryPanel markdown={generateFallbackAnalysis('security', report.discoveryInfo)} />
        )}
        <EmptyStatePanel
          sectionName="Security"
          stackSuggestions={stackSuggestions.length > 0 ? stackSuggestions : undefined}
        />
        <IntelligenceSection
          coverageType="security"
          findings={findingsFor('security')}
          recommendations={recommendationsFor('security')}
          alwaysShow
        />
      </div>
    );
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Security</h1>
      {showAiSummaries && report && (
        <AiSummaryPanel markdown={generateSecuritySummary(report)} />
      )}
              ...
              >
                ...
                >
                  <span className="font-medium text-gray-800 dark:text-gray-100 text-sm flex items-center gap-1">
                    {item.covered ? '✅' : '❌'}
                    {item.category && (
                      <span className="inline-block px-1.5 py-0.5 rounded text-xs font-semibold bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300 mr-1">
                        {item.category}
                      </span>
                    )}
