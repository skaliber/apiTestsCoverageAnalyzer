...
import { generateEndpointSummary, generateFallbackAnalysis } from '../utils/markdownSummaries';

function MethodBadge({ method }: { method: string }) {
  const colorClass = method === 'GET' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-400'
    : method === 'POST' ? 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400'
    : method === 'PUT' || method === 'PATCH' ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-400'
    : method === 'DELETE' ? 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-400'
    : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400';
  return <span className={`inline-block px-1 rounded text-xs font-bold mr-1 ${colorClass}`}>{method}</span>;
}
...
export default function EndpointsPage() {
  ...
  if (!section) {
    return (
      <div className="p-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Endpoints</h1>
        {showAiSummaries && report && (
          <AiSummaryPanel markdown={generateFallbackAnalysis('endpoint', report.discoveryInfo)} />
        )}
        <EmptyStatePanel sectionName="Endpoint" />
        <IntelligenceSection
          coverageType="endpoint"
          findings={findingsFor('endpoint')}
          recommendations={recommendationsFor('endpoint')}
          alwaysShow
        />
      </div>
    );
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Endpoints</h1>
      {showAiSummaries && report && (
        <AiSummaryPanel markdown={generateEndpointSummary(report)} />
      )}
        ...
        <div className="lg:col-span-2">
          ...
          <div className="flex flex-col gap-2">
            {filtered.map((item) => {
              ...
              const path = parts.length > 1 ? parts.slice(1).join(' ') : item.id;
